...
class HeatPumpWaterHeater(ElectricResistanceWaterHeater):
    ...
    def run_thermostat_control(self, use_future_states=False):
        ...
        t_control = (0.75) * t_upper + (0.25) * t_lower # 3/4 , 1/4 was the original
    ...
    def generate_results(self):
        ...
        if self.verbosity >= 7:
            ...
            for i in range(len(tank_temps)):
                results[f'{self.end_use} Tank Temperature Node {i} (C)'] = tank_temps[i]
    
            # Add control temperature
            t_control = 0.75 * tank_temps[self.t_upper_idx] + 0.25 * tank_temps[self.t_lower_idx]
